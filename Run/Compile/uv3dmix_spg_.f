     ...
     &         -pm_v(i,j)*(VFe(i,j)-VFe(i,j-1))
      ...
      do k=1,N-1
        do j=Jstr,Jend
          do i=Istr,Iend+1
          UFe(i,j)=0.125D0*(visc2_sponge_r(i,j)+visc2_sponge_r(i-1,j))
     ...
     &                                           * umask(i,j)
        ...
        do j=Jstr,Jend+1
          do i=Istr,Iend
          VFe(i,j)=0.125D0*(visc2_sponge_r(i,j)+visc2_sponge_r(i,j-1))
     ...
     &                                           * vmask(i,j)
        ...
        do j=Jstr,Jend
          do i=Istr,Iend
            cff1=pm(i,j)*pn(i,j)
            wz(i,j,k,indx)=wz(i,j,k,indx)+dt*cff1*(UFe(i+1,j)-UFe(i,j)
     &                                           + VFe(i,j+1)-VFe(i,j))
          enddo
        enddo
      enddo
      do j=Jstr,Jend
        do i=Istr,Iend+1
        UFe(i,j)=0.25D0*(visc2_sponge_r(i,j)+visc2_sponge_r(i-1,j))
     &                  *pmon_u(i,j)*(Hz(i,j,N  )+Hz(i-1,j,N  ))
     &                        *(wz(i,j,N,nrhs)-wz(i-1,j,N,nrhs))
     &                                         * umask(i,j)
        enddo
      enddo
      do j=Jstr,Jend+1
        do i=Istr,Iend
        VFe(i,j)=0.25D0*(visc2_sponge_r(i,j)+visc2_sponge_r(i,j-1))
     ...
     &                                         * vmask(i,j)
      ...
      do j=Jstr,Jend
        do i=Istr,Iend
          cff1=pm(i,j)*pn(i,j)
          wz(i,j,N,indx)=wz(i,j,N,indx)+dt*cff1*(UFe(i+1,j)-UFe(i,j)
     &                                         + VFe(i,j+1)-VFe(i,j))
        enddo
      enddo
      return
