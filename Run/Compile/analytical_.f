     ...
     &           year2day=365.25D0, day2year=1.D0/365.25D0)
      ...
      common /mask_v/vmask
      common /mask_p2/pmask2
      integer*4 is, k, Istr,Iend,Jstr,Jend, i,j
      ...
      do is=1,Nsrc
        ...
        eps = eps*1e-6
        do k=1,N
          Qsrc(is,k)=(Qbar(is)+eps)*Qshape(is,k)
        enddo
      enddo
      do k=1,N
        do is=1,Nsrc
          Tsrc(is,k,itemp)=Tsrc0(is,itemp)*Qshape(is,k)
          Tsrc(is,k,isalt)=Tsrc0(is,isalt)*Qshape(is,k)
          Tsrc(is,k,itpas)=Tsrc0(is,itpas)*Qshape(is,k)
        enddo
      enddo
      return
      end
     ...
     &    wbry_north_dt(-2:Lm+3+padd_X,N,2)
      ...
      if (.not.WEST_INTER) then
        do j=JstrV-1,JendR
          zetabry_west(j)=-f(IstrR,j)/g*((yr(IstrR,j)-el/2.D0)*u0
     &                                  -(xr(IstrR,j)-xl/2.D0)*v0)
        enddo
        do j=JstrR,JendR
          ubarbry_west(j)=u0
          vbarbry_west(j)=v0
      ...
      if (.not.EAST_INTER) then
        do j=JstrV-1,JendR
          zetabry_east(j)=-f(IendR,j)/g*((yr(IendR,j)-el/2.D0)*u0
     &                                  -(xr(IendR,j)-xl/2.D0)*v0)
        enddo
        do j=JstrR,JendR
          ubarbry_east(j)=u0
          vbarbry_east(j)=v0
      ...
      if (.not.SOUTH_INTER) then
        do i=IstrU-1,IendR
          zetabry_south(i)=-f(i,JstrR)/g*((yr(i,JstrR)-el/2.D0)*u0
     &                                   -(xr(i,JstrR)-xl/2.D0)*v0)
        enddo
        do i=IstrR,IendR
          ubarbry_south(i)=u0
          vbarbry_south(i)=v0
      ...
      if (.not.NORTH_INTER) then
        do i=IstrU-1,IendR
          zetabry_north(i)=-f(i,JendR)/g*((yr(i,JendR)-el/2.D0)*u0
     &                                   -(xr(i,JendR)-xl/2.D0)*v0)
        enddo
        do i=IstrR,IendR
          ubarbry_north(i)=u0
          vbarbry_north(i)=v0
